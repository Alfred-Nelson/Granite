...
class TasksControllerTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @task = create(:task, user: @assignee, task_owner: @creator)
  ...
  def test_should_create_valid_task
    post tasks_path,
      params: { task: { title: "Learn Ruby", task_owner_id: @creator.id, user_id: @assignee.id } },
      headers: @creator_headers
    assert_response :success
    response_json = response.parsed_body
    assert_equal response_json["notice"], t("success", entity: "Task")
  end

  def test_shouldnt_create_task_without_title
    post tasks_path, params: { task: { title: "", task_owner_id: @creator.id, user_id: @assignee.id } },
      headers: @creator_headers
  ...
  def test_not_found_error_rendered_for_invalid_task_slug
    ...
    assert_equal response.parsed_body["error"], t("not_found")
  end
end
