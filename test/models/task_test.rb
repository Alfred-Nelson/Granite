...
class TaskTest < ActiveSupport::TestCase
  def setup
    @user = create(:user)
    @task = create(:task, user: @user, task_owner: @user)
  end

  def test_values_of_created_at_and_updated_at
    task = Task.new(title: "This is a test task", user: @user, task_owner: @user)
  ...
  def test_incremental_slug_generation_for_tasks_with_duplicate_two_worded_titles
    first_task = Task.create!(title: "test task", user: @user, task_owner: @user)
    second_task = Task.create!(title: "test task", user: @user, task_owner: @user)
  ...
  def test_slug_generation_for_tasks_having_titles_one_being_prefix_of_the_other
    first_task = Task.create!(title: "fishing", user: @user, task_owner: @user)
    second_task = Task.create!(title: "fish", user: @user, task_owner: @user)
  ...
  def test_slug_suffix_is_maximum_slug_count_plus_one_if_two_or_more_slugs_already_exist
    title = "test-task"
    first_task = Task.create!(title: title, user: @user, task_owner: @user)
    second_task = Task.create!(title: title, user: @user, task_owner: @user)
    third_task = Task.create!(title: title, user: @user, task_owner: @user)
    fourth_task = Task.create!(title: title, user: @user, task_owner: @user)
    ...
    new_task = Task.create!(title: title, user: @user, task_owner: @user)
  ...
  def test_existing_slug_prefixed_in_new_task_title_doesnt_break_slug_generation
    ...
    existing_task = Task.create!(title: title_having_new_title_as_substring, user: @user, task_owner: @user)
    assert_equal title_having_new_title_as_substring.parameterize, existing_task.slug

    new_task = Task.create!(title: new_title, user: @user, task_owner: @user)
  ...
  def test_having_numbered_slug_substring_in_title_doesnt_affect_slug_generation
    ...
    existing_task = Task.create!(title: title_with_numbered_substring, user: @user, task_owner: @user)
    assert_equal title_with_numbered_substring.parameterize, existing_task.slug

    substring_of_existing_slug = "buy"
    new_task = Task.create!(title: substring_of_existing_slug, user: @user, task_owner: @user)
